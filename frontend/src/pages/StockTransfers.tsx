...
type Transfer = { id: number; code: string; sourceWarehouseId: number; sourceWarehouseName: string; destinationWarehouseId: number; destinationWarehouseName: string; status: string | number; note?: string; createdAt: string; approvedAt?: string; dispatchedAt?: string; receivedAt?: string; completedAt?: string; cancelledAt?: string; details: TransferLine[] };
...
export default function StockTransfers() {
  ...
  const create = async (event: React.FormEvent) => {
    ...
    try {
      await apiClient.post('/api/stock-transfers', { sourceWarehouseId: sourceId, destinationWarehouseId: destinationId, note, details: lines });
  ...
  const act = async (action: string, body?: unknown) => {
    ...
    try { await apiClient.post(`/api/stock-transfers/${selected.id}/${action}`, body); await openDetail(selected.id); await load(); }
